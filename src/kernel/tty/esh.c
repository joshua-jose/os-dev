...
void esh_memset(void* ptr, int value, size_t size){
    uint8_t* mem_area = (uint8_t*)ptr;
    for (size_t i=0;i<size;i++)
        mem_area[i]=value;
    
}

int esh_isalpha(int c) {
...
void *esh_memmove(void *dest, const void *src, size_t n) {
...
static void ins_del(esh_t * esh, char c)
{
    ...
    esh_memmove(&ESH_INSTANCE->buffer[ESH_INSTANCE->ins + sgn],
